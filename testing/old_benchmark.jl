...
function benchmark(backend, func::Expr; filtered=true)
    ...
    while n < 5_000_000 
        ...
        result = eval(Meta.parse("@benchmark $func setup=(begin
            data=$backend.ones($n))
            partial = CUDA.ones(1)
            CUDA.reclaim()
        end"))
...
function benchmark_NVIDIA()
    ...
    ka = benchmark(CUDA, :(CUDA.@sync( mapreducedim(x->x, *, similar(data,(1)), data; init=Float32(0.0))) ;); filtered=false)
    ka_warps = benchmark(CUDA, :(CUDA.@sync( mapreducedim(x->x, *, similar(data,(1)), data; init=Float32(0.0), warps=true)) ;); filtered=false)
    ka_atomics = benchmark(CUDA, :(CUDA.@sync( mapreducedim(x->x, *, similar(data,(1)), data; init=Float32(0.0), atomics=true )) ;); filtered=false)
    ka_warps_atomics = benchmark(CUDA, :(CUDA.@sync( mapreducedim(x->x, *, similar(data,(1)), data; init=Float32(0.0), warps=true, atomics=true));); filtered=false)

    cuda_jl_warps = benchmark(CUDA, :( CUDA.@sync( GPUArrays.mapreducedim!(x->x, *, similar(data,(1)), data; init=Float32(0.0), shuffle=true)); ) ; filtered=false)    
    cuda_jl = benchmark(CUDA, :(CUDA.@sync( GPUArrays.mapreducedim!(x->x, *, similar(data,(1)), data; init=Float32(0.0)) );); filtered=false)
