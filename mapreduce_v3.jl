...
struct Config{              
    ...
    max_concurrent_groups::Val{MAX_CONCURRENT_GROUPS}
    supports_atomics::Val{SUPPORTS_ATOMICS}
    supports_warp_reduce::Val{SUPPORTS_WARP_REDUCE}

    function Config(warpsize, max_groupsize, max_concurrency,max_concurrent_groups , supports_atomics, supports_warp_reduce)
        new{warpsize, max_groupsize, max_concurrency,max_concurrent_groups, supports_atomics, supports_warp_reduce}(
            Val(warpsize), Val(max_groupsize), Val(max_concurrency), Val(max_concurrent_groups),Val(supports_atomics), Val(supports_warp_reduce)
        )
    end
end

struct ReducePolicy{ITEMS_PER_THREAD, GROUPSIZE, USE_WARP_REDUCE, USE_ATOMICS}

    items_per_thread::Val{ITEMS_PER_THREAD}
    groupsize::Val{GROUPSIZE}
    use_warp_reduce::Val{USE_WARP_REDUCE}
    use_atomics::Val{USE_ATOMICS}

    function ReducePolicy(items_per_thread, groupsize, use_warp_reduce, use_atomics)
        new{items_per_thread,groupsize,use_warp_reduce, use_atomics}(Val(items_per_thread), Val(groupsize), Val(use_warp_reduce), Val(use_atomics))
    end
end
...
macro reduce(op, val, neutral, conf) 
    quote
        $__reduce($(esc(:__ctx__)),$(esc(op)), $(esc(val)), $(esc(neutral)), $(esc(Float32)), ($(esc(conf))).supports_warp_reduce)
    end
end
...
function mapreducedim(f::F, op::OP, R, A::Union{AbstractArray,Broadcast.Broadcasted}, partial; init=nothing, atomics = false, warps = false,conf=nothing) where {F, OP}
    ...
    if conf == nothing
        ...
        conf = Config(32, max_groupsize, max_concurrency,gridsize * 4, supports_atomics, supports_warp_reduce)
    end

    policy = ReducePolicy(16,256,false, false)
    ...
    if length(R) == 1
        ...
        if length(A) <= policy.items_per_thread.value * policy.groupsize.value
            
            reduce_kernel(backend, policy.groupsize.value)(f, op, init, 1, R, A, conf, ndrange=policy.groupsize.value)
            ...
            gridsize = cld(length(A), policy.items_per_thread.value)
            gridsize = min(gridsize, policy.groupsize.value * conf.max_concurrent_groups.value)

            #partial = similar(R, (size(R)..., supports_atomics ? 1 : groups_per_grid))
            
            reduce_kernel(backend, policy.groupsize.value)(f, op, init, 1, partial, A, conf, ndrange=gridsize)
...
@benchmark CUDA.@sync( begin mapreducedim(x->x, +, similar($a,(1)),$a,$b; init=Float32(0.0), warps=false) end)
