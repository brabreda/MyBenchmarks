...
macro reduce(op, val, neutral, conf) 
    quote
        $__reduce($(esc(:__ctx__)),$(esc(op)), $(esc(val)), $(esc(neutral)), $(esc(Float32)), Val(($(esc(conf))).supports_warp_reduce))
    end
end
...
function mapreducedim(f::F, op::OP, R, A::Union{AbstractArray,Broadcast.Broadcasted}, partial; init=nothing, atomics = false, warps = false,conf=nothing) where {F, OP}
    ...
    if length(R) == 1
        ...
        if length(A) <= conf.max_groupsize
            
            #reduce_kernel(backend, conf.max_groupsize)(f, op, init, 1, R, A, conf, ndrange=conf.max_groupsize)
            ...
            gridsize = min(length(A), conf.max_concurrency)

            #partial = similar(R, (size(R)..., supports_atomics ? 1 : groups_per_grid))
            
            #reduce_kernel(backend, conf.max_groupsize)(f, op, init, 1, partial, A, conf, ndrange=gridsize)
